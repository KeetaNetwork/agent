...
class SocketController {
    ...
    var handler: ((FileHandleReader, FileHandleWriter) -> Bool)?
    ...
    func configureSocket(at path: String) {
        ...
        DispatchQueue.main.async {
            self.fileHandle?.acceptConnectionInBackgroundAndNotify(forModes: [RunLoop.current.currentMode!])
        }
    }
    ...
    private func socketPort(at path: String) -> SocketPort {
    ...
    @objc private func handleConnectionAccept(notification: Notification) {
    ...
    /// - Parameter notification: A `Notification` that triggered the call.
    @objc private func handleConnectionDataAvailable(notification: Notification) {
